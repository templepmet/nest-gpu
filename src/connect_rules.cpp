...
int NESTGPU::Connect(int i_source_node, int i_target_node, unsigned char port,
                     unsigned char syn_group, float weight, float delay) {
    CheckUncalibrated("Connections cannot be created after calibration");
    net_connection_->Connect(i_source_node, i_target_node, port, syn_group,
                             weight, delay);
...
template <>
int NESTGPU::_SingleConnect<int, int>(int i_source0, int i_source,
                                      int i_target0, int i_target, float weight,
                                      float delay, int i_array,
                                      SynSpec &syn_spec) {
    return net_connection_->Connect(i_source0 + i_source, i_target0 + i_target,
                                    syn_spec.port_, syn_spec.syn_group_, weight,
                                    delay);
}
...
int NESTGPU::_SingleConnect<int, int *>(int i_source0, int i_source,
    ...
    return net_connection_->Connect(i_source0 + i_source,
                                    *(i_target0 + i_target), syn_spec.port_,
                                    syn_spec.syn_group_, weight, delay);
}
...
template <>
int NESTGPU::_RemoteSingleConnect<int>(int i_source, int i_target0,
...
int NESTGPU::Connect(int i_source, int n_source, int i_target, int n_target,
    ...
    return _Connect<int, int>(i_source, n_source, i_target, n_target, conn_spec,
                              syn_spec);
}

int NESTGPU::Connect(int i_source, int n_source, int *target, int n_target,
                     ConnSpec &conn_spec, SynSpec &syn_spec) {
    return _Connect<int, int *>(i_source, n_source, target, n_target, conn_spec,
                                syn_spec);
}
int NESTGPU::Connect(int *source, int n_source, int i_target, int n_target,
                     ConnSpec &conn_spec, SynSpec &syn_spec) {
    return _Connect<int *, int>(source, n_source, i_target, n_target, conn_spec,
                                syn_spec);
}
int NESTGPU::Connect(int *source, int n_source, int *target, int n_target,
                     ConnSpec &conn_spec, SynSpec &syn_spec) {
    return _Connect<int *, int *>(source, n_source, target, n_target, conn_spec,
                                  syn_spec);
}
...
int NESTGPU::RemoteConnect(int i_source_host, NodeSeq source, int i_target_host,
                           NodeSeq target, ConnSpec &conn_spec,
                           SynSpec &syn_spec) {
#ifdef HAVE_MPI
...
int NESTGPU::RemoteConnect(int i_source_host, NodeSeq source, int i_target_host,
    ...
    return _RemoteConnect<int, int *>(rsource, source.n, rtarget, target.size(),
                                      conn_spec, syn_spec);
#else
    throw ngpu_exception("MPI is not available in your build");
#endif
...
int NESTGPU::ConnectPar(int i_source, int n_source, int i_target, int n_target,
                        ConnSpec &conn_spec, SynSpec &syn_spec) {
    return _ConnectPar(i_source, n_source, i_target, n_target, conn_spec,
                       syn_spec);
}
