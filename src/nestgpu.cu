...
int NESTGPU::CreateNodeGroup(int n_node, int n_port)
{
  ...
  connect_mpi_->extern_connection_.insert(it1, n_node, conn_node); // 追加ノード分の領域を広げるだけ
#endif
...
int NESTGPU::Calibrate()
{
  ...
  max_spike_num_ = (int)round(max_spike_num_fact_ * net_connection_->connection_.size() * net_connection_->MaxDelayNum());
  max_spike_num_ = (max_spike_num_ > 1) ? max_spike_num_ : 1;

  max_spike_per_host_ = (int)round(max_spike_per_host_fact_ * net_connection_->connection_.size() * net_connection_->MaxDelayNum());
  ...
  if (mpi_flag_)
  {
    ...
    connect_mpi_->ExternalSpikeInit(connect_mpi_->extern_connection_.size(),
                                    connect_mpi_->mpi_np_, max_spike_per_host_, i_remote_node_0_);
  }
#endif
...
int NESTGPU::Debug()
{
  if (isDebugMode("syndelay_static"))
  {
    ...
    // local
    {
      ...
      for (std::vector<ConnGroup> &conn : net_connection_->connection_)
      { // change syngroup?
...
int NESTGPU::EndSimulation()
{
  ...
  if (verbosity_level_ >= 3)
  {
    ...
    printf("%s  %s: %f(def), %f(host), %f(device)\n", mpirank_str, "poisson_generator_time", poisson_generator_time, poisson_generator_timer->getTimeHost(), poisson_generator_timer->getTimeDevice());
    printf("%s  %s: %f(def), %f(host), %f(device)\n", mpirank_str, "neuron_Update_time", neuron_Update_time, neuron_Update_timer->getTimeHost(), neuron_Update_timer->getTimeDevice());
    printf("%s  %s: %f(def), %f(host), %f(device)\n", mpirank_str, "copy_ext_spike_time", copy_ext_spike_time, copy_ext_spike_timer->getTimeHost(), copy_ext_spike_timer->getTimeDevice());
    ...
    printf("%s  %s: %f(def), %f(host), %f(device)\n", mpirank_str, "ClearGetSpikeArrays_time", ClearGetSpikeArrays_time, ClearGetSpikeArrays_timer->getTimeHost(), ClearGetSpikeArrays_timer->getTimeDevice());
    printf("%s  %s: %f(def), %f(host), %f(device)\n", mpirank_str, "NestedLoop_time", NestedLoop_time, NestedLoop_timer->getTimeHost(), NestedLoop_timer->getTimeDevice());
    printf("%s  %s: %f(def), %f(host), %f(device)\n", mpirank_str, "GetSpike_time", GetSpike_time, GetSpike_timer->getTimeHost(), GetSpike_timer->getTimeDevice());
    printf("%s  %s: %f(def), %f(host), %f(device)\n", mpirank_str, "SpikeReset_time", SpikeReset_time, SpikeReset_timer->getTimeHost(), SpikeReset_timer->getTimeDevice());
...
int NESTGPU::SimulationStep()
{
  ...
  for (unsigned int i = 0; i < node_vect_.size(); i++)
  {
    ...
    {
      node_vect_[i]->SendDirectSpikes(neural_time_, time_resolution_ / 1000.0); // recordDevice
    }
  }
...
std::vector<ConnectionStatus> NESTGPU::GetConnectionStatus(std::vector<ConnectionId>
                                                               &conn_id_vect)
{
                                 ...
                                 std::string param_name, float val)
{
  ...
  if (i_node_0 != i_node || node_vect_[i_group]->n_node_ != n_node)
  {
    throw ngpu_exception(std::string("Group parameter ") + param_name + " can only be set for all and only "
                                                                        " the nodes of the same group");
  }
