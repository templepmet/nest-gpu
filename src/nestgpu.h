...
class NESTGPU {
    ...
    template <class T>
    int _RemoteSingleConnect(int i_source, T target, int i_target, int i_array,
                             SynSpec &syn_spec);
    template <class T>
    int _RemoteSingleConnect(int i_source, T target, int i_target, float weight,
                             float delay, int i_array, SynSpec &syn_spec);

    template <class T1, class T2>
    int _ConnectOneToOne(T1 source, T2 target, int n_node, SynSpec &syn_spec);

    template <class T1, class T2>
    int _ConnectAllToAll(T1 source, int n_source, T2 target, int n_target,
                         SynSpec &syn_spec);

    template <class T1, class T2>
    int _ConnectFixedTotalNumber(T1 source, int n_source, T2 target,
                                 int n_target, int n_conn, SynSpec &syn_spec);

    template <class T1, class T2>
    int _ConnectFixedIndegree(T1 source, int n_source, T2 target, int n_target,
                              int indegree, SynSpec &syn_spec);

    template <class T1, class T2>
    int _ConnectFixedOutdegree(T1 source, int n_source, T2 target, int n_target,
                               int outdegree, SynSpec &syn_spec);

#ifdef HAVE_MPI
    template <class T1, class T2>
    int _RemoteConnect(RemoteNode<T1> source, int n_source,
                       RemoteNode<T2> target, int n_target, ConnSpec &conn_spec,
                       SynSpec &syn_spec);

    template <class T1, class T2>
    int _RemoteConnectOneToOne(RemoteNode<T1> source, RemoteNode<T2> target,
                               int n_node, SynSpec &syn_spec);

    template <class T1, class T2>
    int _RemoteConnectAllToAll(RemoteNode<T1> source, int n_source,
                               RemoteNode<T2> target, int n_target,
                               SynSpec &syn_spec);

    template <class T1, class T2>
    int _RemoteConnectFixedTotalNumber(RemoteNode<T1> source, int n_source,
                                       RemoteNode<T2> target, int n_target,
                                       int n_conn, SynSpec &syn_spec);

    template <class T1, class T2>
    int _RemoteConnectFixedIndegree(RemoteNode<T1> source, int n_source,
                                    RemoteNode<T2> target, int n_target,
                                    int indegree, SynSpec &syn_spec);

    template <class T1, class T2>
    int _RemoteConnectFixedOutdegree(RemoteNode<T1> source, int n_source,
                                     RemoteNode<T2> target, int n_target,
                                     int outdegree, SynSpec &syn_spec);
#endif
   ...
   public:
    ...
    int Connect(int i_source_node, int i_target_node, unsigned char port,
    ...
    int Connect(NodeSeq source, std::vector<int> target, ConnSpec &conn_spec,
                SynSpec &syn_spec);

    int Connect(std::vector<int> source, NodeSeq target, ConnSpec &conn_spec,
                SynSpec &syn_spec);
    ...
    int RemoteConnect(int i_source_host, NodeSeq source, int i_target_host,
                      NodeSeq target, ConnSpec &conn_spec, SynSpec &syn_spec);

    int RemoteConnect(int i_source_host, NodeSeq source, int i_target_host,
                      std::vector<int> target, ConnSpec &conn_spec,
                      SynSpec &syn_spec);

    int RemoteConnect(int i_source_host, std::vector<int> source,
                      int i_target_host, NodeSeq target, ConnSpec &conn_spec,
                      SynSpec &syn_spec);
    ...
    std::vector<ConnectionStatus> GetConnectionStatus(
    ...
    int ActivateRecSpikeTimes(int i_node, int n_node,
