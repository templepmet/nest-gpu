...
int BaseNeuron::Init(int i_node_0, int n_node, int n_port, int i_group,
    ...
    has_dir_conn_ =
        false;  // true if neur. group has outgoing direct connections
    ...
    rec_spike_times_step_ = 0;   // number of time steps for spike times
...
int BaseNeuron::SetIntVar(int i_neuron, int n_neuron, std::string var_name,
    ...
    BaseNeuronSetIntArray<<<(n_neuron + 1023) / 1024, 1024>>>(var_pt, n_neuron,
                                                              1, val);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
...
float *BaseNeuron::GetPortVar(int i_neuron, int n_neuron,
    ...
    gpuErrchk(cudaMemcpy(h_var_arr, d_var_arr,
                         n_neuron * n_port_ * sizeof(float),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaFree(d_var_arr));
...
int BaseNeuron::ActivateRecSpikeTimes(int max_n_rec_spike_times) {
    ...
    if (std::find(int_var_name_.begin(), int_var_name_.end(), s) ==
        ...
        gpuErrchk(
            cudaMalloc(&n_rec_spike_times_cumul_, (n_node_ + 1) * sizeof(int)));
        ...
        gpuErrchk(cudaMalloc(&rec_spike_times_,
                             n_node_ * max_n_rec_spike_times * sizeof(int)));
        gpuErrchk(cudaMalloc(&rec_spike_times_pack_,
                             n_node_ * max_n_rec_spike_times * sizeof(int)));
...
float *BaseNeuron::GetExtNeuronInputSpikes(int *n_node, int *n_port) {
    throw ngpu_exception(
        "Cannot get extern neuron input spikes from this model");
}
...
__global__ void PackSpikeTimesKernel(int n_neuron, int *n_rec_spike_times_cumul,
    ...
    if (array_idx < n_spike_tot) {
        ...
        while ((i_neuron < n_neuron) &&
               (n_rec_spike_times_cumul[i_neuron + 1] ==
...
int BaseNeuron::BufferRecSpikeTimes() {
    ...
    prefix_scan(n_rec_spike_times_cumul_, n_rec_spike_times_, n_node_ + 1,
                true);
...
int BaseNeuron::GetRecSpikeTimes(int **n_spike_times_pt,
    ...
    for (int i_node = 0; i_node < n_node_; i_node++) {
        ...
        for (uint i_buf = 0; i_buf < spike_times_buffer_.size(); i_buf++) {
            ...
            spike_times_vect_[i_node].insert(
                spike_times_vect_[i_node].begin() + k, pt, pt + n_spike);
            k += n_spike;
        }
    }
    for (int i_node = 0; i_node < n_node_; i_node++) {
