...
int NESTGPU_AddMode(char *mode) {
    int ret = 0;
    BEGIN_ERR_PROP { ret = NESTGPU_instance->AddMode(mode); }
    END_ERR_PROP return ret;
}
...
int NESTGPU_Create(char *model_name, int n_neuron, int n_port) {
    ...
    BEGIN_ERR_PROP {
        ...
        NodeSeq neur =
            NESTGPU_instance->Create(model_name_str, n_neuron, n_port);
        ret = neur[0];
    }
    END_ERR_PROP return ret;
}
...
int NESTGPU_CreateRecord(char *file_name, char *var_name_arr[], int *i_node_arr,
    ...
    BEGIN_ERR_PROP {
        ...
        ret = NESTGPU_instance->CreateRecord(
            file_name_str, var_name_vect.data(), i_node_arr, port_arr, n_node);
    }
    END_ERR_PROP return ret;
}
...
int NESTGPU_SetNeuronIntVar(int i_node, int n_neuron, char *var_name, int val) {
    ...
    BEGIN_ERR_PROP {
        ...
        ret = NESTGPU_instance->SetNeuronIntVar(i_node, n_neuron, var_name_str,
                                                val);
    }
    END_ERR_PROP return ret;
}
...
int NESTGPU_SetNeuronArrayVar(int i_node, int n_neuron, char *var_name,
    ...
    BEGIN_ERR_PROP {
        std::string var_name_str = std::string(var_name);
        ret = NESTGPU_instance->SetNeuronVar(i_node, n_neuron, var_name_str,
                                             var, array_size);
    }
    END_ERR_PROP return ret;
}
...
float *NESTGPU_GetNeuronParam(int i_node, int n_neuron, char *param_name) {
    ...
    BEGIN_ERR_PROP {
        std::string param_name_str = std::string(param_name);
        ret =
            NESTGPU_instance->GetNeuronParam(i_node, n_neuron, param_name_str);
    }
    END_ERR_PROP return ret;
}
...
int *NESTGPU_GetNeuronIntVar(int i_node, int n_neuron, char *param_name) {
    ...
    BEGIN_ERR_PROP {
        std::string param_name_str = std::string(param_name);
        ret =
            NESTGPU_instance->GetNeuronIntVar(i_node, n_neuron, param_name_str);
    }
    END_ERR_PROP return ret;
}
...
int NESTGPU_EndSimulation() {
    int ret = 0;
    BEGIN_ERR_PROP { ret = NESTGPU_instance->EndSimulation(); }
    END_ERR_PROP return ret;
}
...
int NESTGPU_MpiId() {
    int ret = 0;
    BEGIN_ERR_PROP { ret = NESTGPU_instance->MpiId(); }
    END_ERR_PROP return ret;
}

int NESTGPU_MpiNp() {
    int ret = 0;
    BEGIN_ERR_PROP { ret = NESTGPU_instance->MpiNp(); }
    END_ERR_PROP return ret;
}
int NESTGPU_ProcMaster() {
    int ret = 0;
    BEGIN_ERR_PROP { ret = NESTGPU_instance->ProcMaster(); }
    END_ERR_PROP return ret;
}
...
int NESTGPU_Connect(int i_source_node, int i_target_node, unsigned char port,
                    unsigned char syn_group, float weight, float delay) {
    int ret = 0;
    BEGIN_ERR_PROP {
        ret = NESTGPU_instance->Connect(i_source_node, i_target_node, port,
...
int NESTGPU_RemoteConnectSeqSeq(int i_source_host, int i_source, int n_source,
                                int i_target_host, int i_target, int n_target) {
    int ret = 0;
    BEGIN_ERR_PROP {
        ret = NESTGPU_instance->RemoteConnect(
            i_source_host, i_source, n_source, i_target_host, i_target,
            n_target, ConnSpec_instance, SynSpec_instance);
    }
    END_ERR_PROP return ret;
}

int NESTGPU_RemoteConnectSeqGroup(int i_source_host, int i_source, int n_source,
...
int NESTGPU_RemoteConnectGroupSeq(int i_source_host, int *i_source,
                                  int n_source, int i_target_host, int i_target,
                                  int n_target) {
    int ret = 0;
    BEGIN_ERR_PROP {
        ret = NESTGPU_instance->RemoteConnect(
...
char **NESTGPU_GetIntVarNames(int i_node) {
    ...
    BEGIN_ERR_PROP {
        ...
        for (unsigned int i = 0; i < var_name_vect.size(); i++) {
            char *var_name =
                (char *)malloc((var_name_vect[i].length() + 1) * sizeof(char));
...
int *NESTGPU_GetSeqGroupConnections(int i_source, int n_source, int *i_target,
                                    int n_target, int syn_group, int *n_conn) {
    int *ret = NULL;
    BEGIN_ERR_PROP {
        std::vector<ConnectionId> conn_id_vect =
...
int *NESTGPU_GetGroupSeqConnections(int *i_source, int n_source, int i_target,
                                    int n_target, int syn_group, int *n_conn) {
    int *ret = NULL;
    BEGIN_ERR_PROP {
        std::vector<ConnectionId> conn_id_vect =
...
char **NESTGPU_GetSynGroupParamNames(int i_syn_group) {
    ...
    BEGIN_ERR_PROP {
        ...
        for (unsigned int i = 0; i < name_vect.size(); i++) {
            char *param_name =
                (char *)malloc((name_vect[i].length() + 1) * sizeof(char));
...
int NESTGPU_GetSynGroupParamIdx(int i_syn_group, char *param_name) {
    ...
    BEGIN_ERR_PROP {
        ...
        ret =
            NESTGPU_instance->GetSynGroupParamIdx(i_syn_group, param_name_str);
    }
    END_ERR_PROP return ret;
}
...
int NESTGPU_GetRecSpikeTimes(int i_node, int n_node, int **n_spike_times_pt,
                             float ***spike_times_pt) {
    int ret = 0;
    BEGIN_ERR_PROP {
        ret = NESTGPU_instance->GetRecSpikeTimes(
...
int NESTGPU_GetExtNeuronInputSpikes(int *n_spikes, int **node, int **port,
    ...
    BEGIN_ERR_PROP {
        ret = NESTGPU_instance->GetExtNeuronInputSpikes(
            n_spikes, node, port, spike_height, include_zeros > 0);
    }
    END_ERR_PROP return ret;
}
...
int NESTGPU_RemoteCreate(int i_host, char *model_name, int n_neuron,
    ...
    BEGIN_ERR_PROP {
        std::string model_name_str = std::string(model_name);
        RemoteNodeSeq rneur = NESTGPU_instance->RemoteCreate(
            i_host, model_name_str, n_neuron, n_port);
