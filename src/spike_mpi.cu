...
__global__ void SendExternalSpike()
{
  ...
  if (i_spike < *ExternalSpikeNum)
  {
    ...
    for (int ith = 0; ith < Nth; ith++)
    {
      ...
      ExternalTargetSpikeNodeId[target_host_id * MaxSpikePerHost + pos] = remote_node_id;
      ExternalTargetSpikeHeight[target_host_id * MaxSpikePerHost + pos] = height;
      ExternalTargetSpikeMinDelay[target_host_id * MaxSpikePerHost + pos] = min_delay;
    }
  }
}
...
int ConnectMpi::ExternalSpikeInit(int n_node, int n_hosts, int max_spike_per_host, int i_remote_node_0)
{
  ...
  h_ExternalTargetSpikeMinDelay = new int[n_hosts * (max_spike_per_host + 1)]; // for overlap
  ...
  // Overlap
  h_ExternalSpikeSourceNode = new int[max_spike_per_host * sizeof(int)];
  h_ExternalTargetSpikeNodeId_immed = new int[n_hosts * (max_spike_per_host + 1)];
  h_ExternalTargetSpikeNodeId_delay = new int[n_hosts * (max_spike_per_host + 1)];
  h_ExternalSourceSpikeNodeId_immed = new int[n_hosts * (max_spike_per_host + 1)];
  h_ExternalSourceSpikeNodeId_delay = new int[n_hosts * (max_spike_per_host + 1)];
  ...
  gpuErrchk(cudaMalloc(&d_ExternalTargetSpikeMinDelay,
  ...
  gpuErrchk(cudaMalloc(&d_ExternalNodeMinDelay, n_node * sizeof(int *))); // for overlap
  ...
  for (int i_source = 0; i_source < n_node; i_source++)
  {
    ...
    if (Nth > 0)
    {
      ...
      gpuErrchk(cudaMalloc(&h_ExternalNodeMinDelay[i_source], Nth * sizeof(int)));
...
int ConnectMpi::ExchangeExternalMinDelay(int n_node, int n_hosts, int i_remote_node_0)
{
  ...
  for (int i = 0; i < n_hosts; ++i)
  {
    n_ex_node_per_host_cumul[i + 1] = n_ex_node_per_host_cumul[i] + n_ex_node_per_host[i];
  }
  ...
  MPI_Alltoall(&n_ex_node_per_host[0], 1, MPI_INT, &n_ex_node_per_host_recv[0], 1, MPI_INT, MPI_COMM_WORLD);
  for (int i = 0; i < n_hosts; ++i)
  {
    n_ex_node_per_host_cumul_recv[i + 1] = n_ex_node_per_host_cumul_recv[i] + n_ex_node_per_host_recv[i];
  }
  int n_ex_node_recv = n_ex_node_per_host_cumul_recv[n_hosts];
  std::vector<int> remote_node_id_recvbuf(n_ex_node_recv + 1);
  MPI_Alltoallv(&remote_node_id_sendbuf[0], &n_ex_node_per_host[0], &n_ex_node_per_host_cumul[0], MPI_INT,
                &remote_node_id_recvbuf[0], &n_ex_node_per_host_recv[0], &n_ex_node_per_host_cumul_recv[0], MPI_INT, MPI_COMM_WORLD);
  ...
  MPI_Alltoallv(&remote_node_mindelay_sendbuf[0], &n_ex_node_per_host_recv[0], &n_ex_node_per_host_cumul_recv[0], MPI_INT,
                &remote_node_mindelay_recvbuf[0], &n_ex_node_per_host[0], &n_ex_node_per_host_cumul[0], MPI_INT, MPI_COMM_WORLD);
...
int ConnectMpi::SendSpikeToRemote(int n_hosts, int max_spike_per_host)
{
  ...
  for (int ih = 0; ih < n_hosts; ih++)
  {
    ...
    MPI_Isend(&h_ExternalTargetSpikeNodeId[array_idx],
              n_spikes, MPI_INT, ih, tag, MPI_COMM_WORLD, &send_mpi_request[ih]);
  }
...
int ConnectMpi::RecvSpikeFromRemote(int n_hosts, int max_spike_per_host)
{
  ...
  for (int i = 0; i < n_hosts; ++i)
  {
    s += std::to_string(h_ExternalSourceSpikeNum[i]) + ", ";
  }
  printf("mpi_id:%d, %s\n", mpi_id_, s.c_str());
...
int ConnectMpi::SendSpikeToRemoteOverlap(int n_hosts, int max_spike_per_host)
{
  ...
  gpuErrchk(cudaMemcpy(h_ExternalTargetSpikeMinDelay,
                       d_ExternalTargetSpikeMinDelay,
                       (n_hosts * max_spike_per_host) * sizeof(int),
                       cudaMemcpyDeviceToHost));
  std::vector<std::vector<int>> send_buf_immed(n_hosts);
  std::vector<std::vector<int>> send_buf_delay(n_hosts);
  for (int i = 0; i < n_hosts; ++i)
  {
    ...
    {
      if (h_ExternalTargetSpikeMinDelay[i * max_spike_per_host + j] == 0)
      {
        send_buf_immed[i].push_back(h_ExternalTargetSpikeNodeId[idx + j]);
      }
      else
      {
        send_buf_delay[i].push_back(h_ExternalTargetSpikeNodeId[idx + j]);
      }
    }
  }
  std::vector<MPI_Status> send_mpi_status(n_hosts);

  // immed
  for (int i = 0; i < n_hosts; ++i)
  {
    MPI_Isend(send_buf_immed[i].data(), send_buf_immed[i].size(), MPI_INT, i, tag_immed, MPI_COMM_WORLD, &send_mpi_request[i]);
  }
  MPI_Waitall(n_hosts, &send_mpi_request[0], &send_mpi_status[0]);
  // delay
  for (int i = 0; i < n_hosts; ++i)
  {
    MPI_Isend(send_buf_delay[i].data(), send_buf_delay[i].size(), MPI_INT, i, tag_delay, MPI_COMM_WORLD, &send_mpi_request[i]);
  }
  MPI_Waitall(n_hosts, &send_mpi_request[0], &send_mpi_status[0]);
...
int ConnectMpi::RecvSpikeFromRemoteOverlap(int n_hosts, int max_spike_per_host)
{
  ...
  std::vector<std::vector<int>> recv_buf_immed(n_hosts, std::vector<int>(max_spike_per_host));
  std::vector<std::vector<int>> recv_buf_delay(n_hosts, std::vector<int>(max_spike_per_host));
  std::vector<MPI_Request> recv_mpi_request_immed(n_hosts);
  std::vector<MPI_Request> recv_mpi_request_delay(n_hosts);
  std::vector<MPI_Status> recv_mpi_status_immed(n_hosts);
  std::vector<MPI_Status> recv_mpi_status_delay(n_hosts);

  // immed
  for (int i = 0; i < n_hosts; ++i)
  {
    MPI_Irecv(&recv_buf_immed[i][0], max_spike_per_host, MPI_INT, i, tag_immed, MPI_COMM_WORLD, &recv_mpi_request_immed[i]);
  }
  MPI_Waitall(n_hosts, &recv_mpi_request_immed[0], &recv_mpi_status_immed[0]);
  // delay
  for (int i = 0; i < n_hosts; ++i)
  {
    MPI_Irecv(&recv_buf_delay[i][0], max_spike_per_host, MPI_INT, i, tag_delay, MPI_COMM_WORLD, &recv_mpi_request_delay[i]);
  }
  MPI_Waitall(n_hosts, &recv_mpi_request_delay[0], &recv_mpi_status_delay[0]);

  for (int i = 0; i < n_hosts; i++)
  {
    MPI_Get_count(&recv_mpi_status_immed[i], MPI_INT, &count_immed);
    MPI_Get_count(&recv_mpi_status_delay[i], MPI_INT, &count_delay);
    std::copy(&recv_buf_immed[i][0], &recv_buf_immed[i][count_immed], h_ExternalSourceSpikeNodeId + (i * max_spike_per_host));
    std::copy(&recv_buf_delay[i][0], &recv_buf_delay[i][count_delay], h_ExternalSourceSpikeNodeId + (i * max_spike_per_host) + count_immed);
                                    ...
                                    int i_remote_node_0)
{
  ...
  if (n_spike_tot > 0)
  {
    ...
    // convert node group indexes to spike buffer indexes
    // by adding the index of the first node of the node group
    AddOffset<<<(n_spike_tot + 1023) / 1024, 1024>>>(n_spike_tot, d_ExternalSourceSpikeNodeId, i_remote_node_0);
                                ...
                                int n_spike_tot, int max_spike_per_host)
{
  ...
  if (array_idx < n_spike_tot)
  {
    ...
    while ((i_host < n_hosts) && (ExternalTargetSpikeCumul[i_host + 1] == ExternalTargetSpikeCumul[i_host]))
    {
      i_host++;
      if (i_host == n_hosts)
        return;
...
int ConnectMpi::JoinSpikes(int n_hosts, int max_spike_per_host)
{
  ...
  for (int ih = 0; ih < n_hosts; ih++)
  {
    h_ExternalTargetSpikeCumul[ih + 1] = h_ExternalTargetSpikeCumul[ih] + h_ExternalTargetSpikeNum[ih];
  }
