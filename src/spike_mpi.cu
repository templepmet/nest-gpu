...
int *d_ExternalSourceSpikeNodeId_recv;
...
int ConnectMpi::ExternalSpikeInit(int n_node, int n_hosts,
    ...
    // gpuErrchk(cudaMalloc(&d_ExternalSourceSpikeNum,
    // n_hosts*sizeof(int)));
    gpuErrchk(cudaMalloc(&d_ExternalSourceSpikeNodeId,
                         n_hosts * max_spike_per_host * sizeof(int)));
    gpuErrchk(cudaMalloc(&d_ExternalSourceSpikeNodeId_recv,
                         n_hosts * max_spike_per_host * sizeof(int)));
...
std::vector<MPI_Request> persistent_request[2];
std::vector<MPI_Status> persistent_status[2];
cudaStream_t delay_stream;
void *d_temp_storage;
...
int ConnectMpi::ExternalSpikeInitOverlap(int n_node, int n_hosts,
                                         ...
                                         int i_remote_node_0) {
    printf("Rank:%d, max_spike_per_host:%d\n", mpi_id_, max_spike_per_host);
    ...
    // for CUB scan
    cudaStreamCreateWithFlags(&delay_stream, cudaStreamNonBlocking);
    cub::DeviceScan::ExclusiveSum(
        d_temp_storage, temp_storage_bytes, d_ExternalTargetSpikeNum_delay,
        d_ExternalTargetSpikeCumul_delay, n_hosts + 1);
    cudaMalloc(&d_temp_storage, temp_storage_bytes);

    if (isMode("comm_persistent")) {
        // for persistent communication
        persistent_request[0].resize(n_hosts * 2);
        persistent_request[1].resize(n_hosts * 2);
        persistent_status[0].resize(n_hosts * 2);
        persistent_status[1].resize(n_hosts * 2);
        for (int i = 0; i < n_hosts; ++i) {
            MPI_Send_init(
                &h_ExternalTargetSpikeNodeId_delay[0][i * max_spike_per_host],
                max_spike_per_host, MPI_INT, i, tag_delay, MPI_COMM_WORLD,
                &persistent_request[0][i]);
            MPI_Send_init(
                &h_ExternalTargetSpikeNodeId_delay[1][i * max_spike_per_host],
                max_spike_per_host, MPI_INT, i, tag_delay, MPI_COMM_WORLD,
                &persistent_request[1][i]);
        }
        for (int i = 0; i < n_hosts; i++) {
            MPI_Recv_init(
                &h_ExternalSourceSpikeNodeId_delay[i * max_spike_per_host],
                max_spike_per_host, MPI_INT, i, tag_delay, MPI_COMM_WORLD,
                &persistent_request[0][n_hosts + i]);
            MPI_Recv_init(
                &h_ExternalSourceSpikeNodeId_delay[i * max_spike_per_host],
                max_spike_per_host, MPI_INT, i, tag_delay, MPI_COMM_WORLD,
                &persistent_request[1][n_hosts + i]);
        }
    }
...
int ConnectMpi::SendRecvSpikeRemoteCuda(int n_hosts, int max_spike_per_host,
                                        int i_remote_node_0, long long it_,
                                        long long Nt_) {
    int tag = 1, count;
    Other_timer->startRecord();
    ...
    //     h_ExternalTargetSpikeNodeId_immed, h_ExternalTargetSpikeNum_immed,
    //     h_ExternalTargetSpikeCumul_immed, MPI_INT,
    //     h_ExternalSourceSpikeNodeId_immed,
    //     h_ExternalSourceSpikeNum_recvcounts,
    //     h_ExternalSourceSpikeCumul_rdispls, MPI_INT,
    //     MPI_COMM_WORLD);  // 受信サイズが必要
    MPI_Alltoall(h_ExternalTargetSpikeNum, 1, MPI_INT, h_ExternalSourceSpikeNum,
    ...
    for (int i = 0; i < n_hosts; i++) {
        // int recv_count = (i == mpi_id_ ? 0 : max_spike_per_host);
        MPI_Irecv(&d_ExternalSourceSpikeNodeId_recv[i * max_spike_per_host],
                  h_ExternalSourceSpikeNum[i], MPI_INT, i, tag, MPI_COMM_WORLD,
                  &recv_mpi_request[i]);
    }
    MPI_Waitall(n_hosts, send_mpi_request, send_mpi_status);
    MPI_Waitall(n_hosts, recv_mpi_request, recv_mpi_status);
    ...
    for (int i = 0; i < n_hosts; ++i) {
        // MPI_Get_count(&recv_mpi_status[i], MPI_INT, &count);
        // h_ExternalSourceSpikeNum[i] = count;
        n_spike_tot += h_ExternalSourceSpikeNum[i];
    }
    // h_ExternalSourceSpikeNum[mpi_id_] = 0;
    UnpackRecvSpike_timer->stopRecordHost();
    ...
    if (n_spike_tot > 0) {
        ...
        for (int i = 0; i < n_hosts; ++i) {
            gpuErrchk(cudaMemcpyAsync(
                d_ExternalSourceSpikeNodeId + dpos,
                d_ExternalSourceSpikeNodeId_recv + i * max_spike_per_host,
                sizeof(int) * h_ExternalSourceSpikeNum[i],
                cudaMemcpyDeviceToDevice));
            dpos += h_ExternalSourceSpikeNum[i];
        }
        AddOffset<<<(n_spike_tot + 1023) / 1024, 1024>>>(
            n_spike_tot, d_ExternalSourceSpikeNodeId, i_remote_node_0);
        PushSpikeFromRemote<<<(n_spike_tot + 1023) / 1024, 1024>>>(
...
__global__ void preProcessingJoin(int *d_SpikeNum, int *d_SpikeNodeId,
                                  int max_spike_per_host) {
    int i_host = threadIdx.x;
    if (i_host < NExternalTargetHost) {
        d_SpikeNodeId[i_host * max_spike_per_host] = d_SpikeNum[i_host];
        d_SpikeNum[i_host]++;
    }
}
...
int ConnectMpi::JoinSpikesOverlap(int n_hosts, int max_spike_per_host,
                                  ...
                                  int *d_SpikeNodeId, int *d_SpikeNodeIdJoin) {
    preProcessingJoin<<<1, n_hosts>>>(d_SpikeNum, d_SpikeNodeId,
                                      max_spike_per_host);
...
int JoinSpikesAsync(int n_hosts, int max_spike_per_host, int *d_SpikeCumul,
                    ...
                    int *d_SpikeNodeId, int *d_SpikeNodeIdJoin,
                    cudaStream_t stream) {
    preProcessingJoin<<<1, n_hosts, 0, stream>>>(d_SpikeNum, d_SpikeNodeId,
                                                 max_spike_per_host);
    cub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes,
                                  d_SpikeNum, d_SpikeCumul, n_hosts + 1,
                                  stream);
