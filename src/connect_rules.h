...
int NESTGPU::_Connect(T1 source, int n_source, T2 target, int n_target,
    ...
    switch (conn_spec.rule_) {
        ...
        case FIXED_TOTAL_NUMBER:
            return _ConnectFixedTotalNumber<T1, T2>(
                source, n_source, target, n_target, conn_spec.total_num_,
                syn_spec);
            break;
        case FIXED_INDEGREE:
            return _ConnectFixedIndegree<T1, T2>(source, n_source, target,
                                                 n_target, conn_spec.indegree_,
                                                 syn_spec);
            break;
        case FIXED_OUTDEGREE:
            return _ConnectFixedOutdegree<T1, T2>(
                source, n_source, target, n_target, conn_spec.outdegree_,
                syn_spec);
            break;
        default:
...
int NESTGPU::_SingleConnect(T1 source, int i_source, T2 target, int i_target,
    ...
    return _SingleConnect<T1, T2>(source, i_source, target, i_target, weight,
                                  delay, i_array, syn_spec);
}
...
template <class T>
int NESTGPU::_RemoteSingleConnect(int i_source, T target, int i_target,
                                  int i_array, SynSpec &syn_spec) {
    float weight;
    if (syn_spec.weight_array_ != NULL) {
        weight = syn_spec.weight_array_[i_array];
    ...
    return _RemoteSingleConnect<T>(i_source, target, i_target, weight, delay,
                                   i_array, syn_spec);
}
...
template <class T1, class T2>
int NESTGPU::_ConnectAllToAll(T1 source, int n_source, T2 target, int n_target,
                              SynSpec &syn_spec) {
#ifdef _OPENMP
...
template <class T1, class T2>
int NESTGPU::_ConnectFixedTotalNumber(T1 source, int n_source, T2 target,
                                      int n_target, int n_conn,
                                      SynSpec &syn_spec) {
...
template <class T1, class T2>
int NESTGPU::_ConnectFixedIndegree(T1 source, int n_source, T2 target,
                                   int n_target, int indegree,
                                   SynSpec &syn_spec) {
            ...
            if (itn < n_target) {
                ...
                if (n_source < method_thresh * indegree) {  // vecchio metodo
                    ...
                    for (int i = 0; i < indegree; i++) {
                        int j =
                            i + rnd[i * THREAD_MAXNUM + ith] % (n_source - i);
                        if (j != i) {
                ...
                for (int i = 0; i < indegree; i++) {
                    ...
                    omp_set_lock(&(lock[isn]));
#endif
                    _SingleConnect<T1, T2>(source, isn, target, itn, i_array,
                                           syn_spec);
#ifdef _OPENMP
                    omp_unset_lock(&(lock[isn]));
#endif
...
template <class T1, class T2>
int NESTGPU::_ConnectFixedOutdegree(T1 source, int n_source, T2 target,
                                    int n_target, int outdegree,
                                    SynSpec &syn_spec) {
    const int method_thresh = 5;
            ...
            if (isn < n_source) {
                ...
                if (n_target < method_thresh * outdegree) {  // choose method
                    ...
                    for (int i = 0; i < outdegree; i++) {
                        int j =
                            i + rnd[i * THREAD_MAXNUM + ith] % (n_target - i);
                        if (j != i) {
                            std::swap(int_vect[i], int_vect[j]);
                        }
                    }
                    ...
                    for (int i = 0; i < outdegree; i++) {
                        ...
                        int j;
                        do {
                            j = rnd[(i1 * outdegree + i) * THREAD_MAXNUM +
                                    ith] %
                                n_target;
                            // https://riptutorial.com/cplusplus/example/7270/using-a-sorted-vector-for-fast-element-lookup
                            // check if j is in target_vect
...
template <class T1, class T2>
int NESTGPU::_RemoteConnect(RemoteNode<T1> source, int n_source,
    ...
    switch (conn_spec.rule_) {
        case ONE_TO_ONE:
            ...
            return _RemoteConnectOneToOne<T1, T2>(source, target, n_source,
                                                  syn_spec);
            break;
        case ALL_TO_ALL:
            return _RemoteConnectAllToAll<T1, T2>(source, n_source, target,
                                                  n_target, syn_spec);
            break;
        case FIXED_TOTAL_NUMBER:
            ...
            break;
        case FIXED_INDEGREE:
            return _RemoteConnectFixedIndegree<T1, T2>(
                source, n_source, target, n_target, conn_spec.indegree_,
                syn_spec);
            break;
        case FIXED_OUTDEGREE:
            return _RemoteConnectFixedOutdegree<T1, T2>(
                source, n_source, target, n_target, conn_spec.outdegree_,
                syn_spec);
            break;
        default:
...
template <class T1, class T2>
int NESTGPU::_RemoteConnectOneToOne(RemoteNode<T1> source,
                                    RemoteNode<T2> target, int n_node,
                                    SynSpec &syn_spec) {
    if (MpiId() == source.i_host_ && source.i_host_ == target.i_host_) {
        return _ConnectOneToOne<T1, T2>(source.i_node_, target.i_node_, n_node,
                                        syn_spec);
        ...
        if (MpiId() == target.i_host_) {
            ...
            for (int in = 0; in < n_node; in++) {
                ...
                _RemoteSingleConnect<T2>(i_remote_node, target.i_node_, in, in,
                                         syn_spec);
            }
            ...
            for (int in = 0; in < n_node; in++) {
                ...
                for (std::vector<ExternalConnectionNode>::iterator it =
                         connect_mpi_->extern_connection_[i_source_node]
                             .begin();
                     it < connect_mpi_->extern_connection_[i_source_node].end();
                     it++) {
                    if ((*it).target_host_id ==
                        target.i_host_)  // 辺が異なってもhost_idで同一視する？
                    {
                ...
                if (i_remote_node == -1) {
                    ...
                    i_new_remote_node++;
                    ExternalConnectionNode conn_node = {target.i_host_,
                                                        i_remote_node};
                    connect_mpi_->extern_connection_[i_source_node].push_back(
                        conn_node);
                }
                i_remote_node_arr[in] = i_remote_node;
            }
            connect_mpi_->MPI_Send_int(&i_new_remote_node, 1, target.i_host_);
            connect_mpi_->MPI_Send_int(i_remote_node_arr, n_node,
                                       target.i_host_);
        }
...
template <class T1, class T2>
int NESTGPU::_RemoteConnectAllToAll(RemoteNode<T1> source, int n_source,
                                    RemoteNode<T2> target, int n_target,
                                    SynSpec &syn_spec) {
    if (MpiId() == source.i_host_ && source.i_host_ == target.i_host_) {
                ...
                for (int i = 0; i < n_source; i++) {
                    ...
                    for (std::vector<ExternalConnectionNode>::iterator it =
                             connect_mpi_->extern_connection_[i_source_node]
                                 .begin();
                         it <
                         connect_mpi_->extern_connection_[i_source_node].end();
                         it++) {
...
template <class T1, class T2>
int NESTGPU::_RemoteConnectFixedTotalNumber(RemoteNode<T1> source, int n_source,
                                            RemoteNode<T2> target, int n_target,
    ...
    MPI_Barrier(MPI_COMM_WORLD);  // 犯人

    return 0;
}

template <class T1, class T2>
int NESTGPU::_RemoteConnectFixedIndegree(RemoteNode<T1> source, int n_source,
                                         RemoteNode<T2> target, int n_target,
...
template <class T1, class T2>
int NESTGPU::_RemoteConnectFixedOutdegree(RemoteNode<T1> source, int n_source,
                                          RemoteNode<T2> target, int n_target,
    ...
    if (MpiId() == source.i_host_ && source.i_host_ == target.i_host_) {
        ...
        if (MpiId() == target.i_host_) {
            ...
            connect_mpi_->MPI_Send_int(&i_new_remote_node, 1, target.i_host_);
            connect_mpi_->MPI_Send_int(i_remote_node_arr, n_source,
                                       target.i_host_);
        }
        delete[] i_remote_node_arr;
    }
