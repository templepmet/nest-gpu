...
def compute_Model_params(label, mode="default"):
    """
    ...
    binzegger_I_pops = [
        binzegger_pops[i]
        for i in range(len(binzegger_pops))
        if binzegger_pops[i].find("b") != -1
    ]
    binzegger_E_pops = [
        binzegger_pops[i]
        for i in range(len(binzegger_pops))
    ...
    if mode == "default":
        ...
        with open(
            os.path.join("simulation_result", label, "custom_config.json"),
            "r",
        ) as f:
    ...
    for area in area_list:
        ...
        rho_syn[area] = (
            av_indegree_V1
            * neuronal_numbers_fullscale["V1"]["total"]
    ...
    for a in neuronal_numbers_fullscale:
        ...
        for pop in neuronal_numbers_fullscale[a]:
            neuronal_numbers[a][pop] = (
                neuronal_numbers_fullscale[a][pop] / Area_surfaces[a] * surface
            )
    ...
    r_PD14 = np.sqrt(1.0 / np.pi)
    C_prime_mean_PD14 = (
        2.0
        / (r_PD14**2)
        * C0
        * sigma**2
        * (1.0 - np.exp(-(r_PD14**2) / (2 * sigma**2)))
    )
    ...
    for area, target_pop, source_pop in product(
        ...
        C = (
            Intra_areal[target_pop][source_pop]
            * C_prime_fullscale_mean[area]
            / C_prime_mean_PD14
        )
        if Area_surfaces[area] < 100.0:  # Limit to choose between np.log and log_approx
            K = int(
                round(
                    np.log(1.0 - C)
                    / np.log(
                        ...
                        - 1.0
                        / (
                            num_V1[target_pop]["neurons"]
                            * num_V1[source_pop]["neurons"]
                            * Area_surfaces[area] ** 2
                        )
                    )
                )
    ...
    for area in area_list:
        Intrinsic_FLN_completed_fullscale[area] = {
            "mean": mean_Intrinsic_FLN,
            "error": mean_Intrinsic_error,
        }
    ...
    for area, target_pop, source_pop in product(
        ...
        C = Intra_areal[target_pop][source_pop] * C_prime_mean[area] / C_prime_mean_PD14
        if surface < 100.0:  # Limit to choose between np.log and log_approx
            K = int(
                round(
                    np.log(1.0 - C)
                    / np.log(
    ...
    for area in area_list:
        ...
        for pop in population_list:
            for pop2 in population_list:
                if Indegree_prime_fullscale[area][pop][pop2] > 0.0:
                    average_relation_indegrees.append(
                        Indegree_prime[area][pop][pop2]
                        / Indegree_prime_fullscale[area][pop][pop2]
                    )
        Intrinsic_FLN_completed[area] = {
            "mean": mean_Intrinsic_FLN * np.mean(average_relation_indegrees),
            "error": mean_Intrinsic_error,
        }
    ...
    for area in area_list:
        ...
        for target_pop in population_list:
            for source_pop in population_list:
                Nsyn_int_prime += (
                    Indegree_prime[area][target_pop][source_pop]
                    * neuronal_numbers[area][target_pop]
                )
        conversion_factor[area] = (
            Intrinsic_FLN_completed[area]["mean"]
            * rho_syn[area]
            * surface
            * total_thicknesses[area]
            / Nsyn_int_prime
        )
    ...
    for area in area_list:
        ...
        for target_pop in population_list:
            for source_pop in population_list:
                Nsyn_int_prime += (
                    Indegree_prime_fullscale[area][target_pop][source_pop]
                    * neuronal_numbers_fullscale[area][target_pop]
                )
        conversion_factor_fullscale[area] = (
            Intrinsic_FLN_completed_fullscale[area]["mean"]
            * rho_syn[area]
            * Area_surfaces[area]
            * total_thicknesses[area]
            / Nsyn_int_prime
    ...
    def num_IA_synapses(area, target_pop, source_pop, area_model="micro"):
        ...
        if area_model == "micro":
            ...
            In_degree = Indegree_prime[area][target_pop][source_pop]
        ...
        if area_model == "real":
            ...
            In_degree = Indegree_prime_fullscale[area][target_pop][source_pop]
    ...
    for target_area in area_list:
        ...
        for target_pop in population_list:
            ...
            if neuronal_numbers[target_area][target_pop] != 0.0:
                for source_pop in population_list:
                    micro_in_degree = (
                        num_IA_synapses(target_area, target_pop, source_pop)
                        / neuronal_numbers[target_area][target_pop]
                    )
                    real_in_degree = (
                        num_IA_synapses(
                            target_area, target_pop, source_pop, area_model="real"
                        )
                        / neuronal_numbers_fullscale[target_area][target_pop]
                    )
                    syn += (real_in_degree - micro_in_degree) * neuronal_numbers[
                        target_area
                    ][target_pop]
            s_area += syn
    ...
    for target_area in FLN_EDR_completed:
        ...
        cc_proportion = (
            1.0 - Intrinsic_FLN_completed_fullscale[target_area]["mean"] - 0.013
        )
        norm_factor = cc_proportion / sum(FLN_EDR_completed[target_area].values())
        for source_area in FLN_EDR_completed[target_area]:
            FLN_completed[target_area][source_area] = (
                norm_factor * FLN_EDR_completed[target_area][source_area]
            )
    ...
    for cb in binzegger_data:
        ...
        if cell_layer not in ["", "1"]:
            if cb in binzegger_E_pops:
                relative_numbers_binzegger[cell_layer + "E"] += (
                    binzegger_data[cb]["occurrence"] / s
                )
            if cb in binzegger_I_pops:
                relative_numbers_binzegger[cell_layer + "I"] += (
                    binzegger_data[cb]["occurrence"] / s
                )

    # Determine the relative numbers of the 8 populations in V1
    relative_numbers_model = {
        "23E": 0.0,
        "23I": 0.0,
        "4E": 0.0,
        "4I": 0.0,
    ...
    for pop in neuronal_numbers["V1"]:
        relative_numbers_model[pop] = (
            neuronal_numbers["V1"][pop] / neuronal_numbers["V1"]["total"]
        )
        ...
        if i != "1I":
            for v in binzegger_data[cb]["syn_dict"]:
                if v in num_cc_synapses:
                    num_ratio = (
                        relative_numbers_model[i] / relative_numbers_binzegger[i]
                    )
                    cc_syn_num = (
                        binzegger_data[cb]["syn_dict"][v]["corticocortical"]
                        / 100.0
                ...
                if i != "1I":  # We do not model cell types in layer 1
                    ...
                    if v in synapse_to_cell_body_basis:
                        if i in synapse_to_cell_body_basis[v]:
                            synapse_to_cell_body_basis[v][i] += cond_prob
                        else:
                            synapse_to_cell_body_basis[v].update({i: cond_prob})
                    else:
                        synapse_to_cell_body_basis.update({v: {i: cond_prob}})
    ...
    for layer in synapse_to_cell_body["TH"]:
        ...
        for pop in ["23E", "5E", "6E"]:
            if "4E" in synapse_to_cell_body["TH"][layer]:
                if pop in synapse_to_cell_body["TH"][layer]:
                    synapse_to_cell_body["TH"][layer][pop] += (
                        synapse_to_cell_body["TH"][layer]["4E"]
                        * laminar_thicknesses["TH"][pop[0:-1]]
                        / l
                    )
                else:
                    synapse_to_cell_body["TH"][layer][pop] = (
        ...
        for pop in ["23I", "5I", "6I"]:
            if "4I" in synapse_to_cell_body["TH"][layer]:
                if pop in synapse_to_cell_body["TH"][layer]:
                    synapse_to_cell_body["TH"][layer][pop] += (
                        synapse_to_cell_body["TH"][layer]["4I"]
                        * laminar_thicknesses["TH"][pop[0:-1]]
                        / l
                    )
                else:
    ...
    def num_CC_synapses(target_area, target_pop, source_area, source_pop):
        ...
        if (
            source_area in Coco_Data[target_area]
            and source_pop not in ["4I", "4E"]
            and neuronal_numbers[target_area][target_pop] != 0
            and source_pop not in ["23I", "4I", "5I", "6I"]
        ):
            ...
            Nsyn_tot = (
                rho_syn[target_area]
                * Area_surfaces[target_area]
                * total_thicknesses[target_area]
            )
            ...
            if Coco_Data[target_area][source_area]["source_pattern"] is not None:
                sp = np.array(
                    Coco_Data[target_area][source_area]["source_pattern"],
                    dtype=np.float,
                )
                ...
                if source_pop in origin_patterns["S"]:
                    ...
                    if np.any(sp[-2:] != 0):
                        ...
                        if sp[index] != 0:
                            X = 1.0 - SLN_value
                            Y = 10 ** (sp[index]) / np.sum(
                                10 ** sp[-2:][np.where(sp[-2:] != 0)]
                            )
                        else:
                            X = 0.0
                            Y = 0.0
            ...
            # otherwise, use neuronal numbers
                ...
                if source_pop in origin_patterns["S"]:
                    ...
                    for i in origin_patterns["I"]:
                        infra_neurons += neuronal_numbers_fullscale[source_area][i]
            ...
            if Coco_Data[target_area][source_area]["target_pattern"] is not None:
                tp = np.array(
                    Coco_Data[target_area][source_area]["target_pattern"],
                    dtype=np.float,
                )
                ...
                if -1 in tp:
                    if (
                        ...
                        and SLN_Data[target_area][source_area] <= 0.65
                    ):
                ...
                for i in range(len(T)):
                    if T[i] in [2, 3]:
                        ...
                        syn_layer = str(T[i])
                    Z = 10 ** tp[np.where(tp > 0.0)[0]][i] / p_T
                    ...
                    if target_pop in synapse_to_cell_body[target_area][syn_layer]:
                        if syn_layer == "1":
                            ...
                            if "1" in T:
                                Z = (
                                    0.5
                                    * laminar_thicknesses[target_area][syn_layer]
                                    / p_T
                                )
                            else:
                                Z = laminar_thicknesses[target_area][syn_layer] / p_T
                        Nsyn += (
                            synapse_to_cell_body[target_area][syn_layer][target_pop]
                            * Nsyn_tot
                            * FLN_BA
    ...
    for target_area, target_pop, source_area, source_pop in product(
        area_list, population_list, area_list, population_list
    ):
        if target_area != source_area:
            ...
            if N != 0:
                N_syn = (
                    num_CC_synapses(target_area, target_pop, source_area, source_pop)
                    / N_fullscale
                    * N
                )
            else:
        ...
        for target_area in area_list:
            for source_area in area_list:
                if (
                    ...
                    and source_area in Coco_Data[target_area]
                    and SLN_Data[target_area][source_area] < 0.35
                ):
                    syn_I = 0.0
                    syn_E = 0.0
                    for target_pop in synapse_numbers[target_area]:
                        for source_pop in synapse_numbers[target_area][target_pop][
                            source_area
                        ]:
                            if target_pop.find("E") > -1:
                                syn_E += synapse_numbers[target_area][target_pop][
                                    source_area
                                ][source_pop]
                            else:
                                syn_I += synapse_numbers[target_area][target_pop][
                                    source_area
                                ][source_pop]
                        ...
                        if alpha_I != 0.0 and alpha_E != 0.0:
                            for target_pop in synapse_numbers[target_area]:
                                for source_pop in synapse_numbers[target_area][
                                    ...
                                    if target_pop.find("E") > -1:
                                        synapse_numbers[target_area][target_pop][
                                            source_area
                                        ][source_pop] = (
                                            E_specific_factor / alpha_E * N_syn
                                        )
                                    else:
                                        synapse_numbers[target_area][target_pop][
                                            source_area
                                        ][source_pop] = (
                                            (1.0 - E_specific_factor) / alpha_I * N_syn
                                        )
    ...
    for target_area in area_list:
        ...
        CC_synapses = 0.0
        for target_pop, source_area, source_pop in product(
            population_list, area_list, population_list
        ):
            if source_area != target_area:
                CC_synapses += synapse_numbers[target_area][target_pop][source_area][
                    source_pop
                ]
        ext_syn = (
            N_syn_tot * (1.0 - Intrinsic_FLN_completed[target_area]["mean"])
            - CC_synapses
        )
    ...
    for area in area_list:
        ...
        for i in range(len(structure[area])):
            nonvisual_fraction_matrix[i] = (
                1.0 / len(structure[area]) * np.ones(len(structure[area]))
            )
            nonvisual_fraction_matrix[i][i] -= 1

        for i in range(len(structure[area])):
            nonvisual_fraction_matrix[-1][i] = neuronal_numbers[area][
                structure[area][i]
            ]
        ...
        vector[-1] = ext_syn
        solution, residues, rank, s = np.linalg.lstsq(nonvisual_fraction_matrix, vector)
    ...
    for target_area in area_list:
        for target_pop in synapse_numbers[target_area]:
            if target_pop in ["5E"]:
                synapse_numbers[target_area][target_pop]["external"]["external"] = (
                    fac_nu_ext_5E
                    * synapse_numbers[target_area][target_pop]["external"]["external"]
                )
            if target_pop in ["6E"]:
    ...
    PSC_e_over_PSP_e = (
        ...
        * (
            ...
            - (tau_m / tau_syn_ex) ** (-tau_syn_ex / (tau_m - tau_syn_ex))
        )
    ) ** (-1)
    PSC_i_over_PSP_i = (
        C_m ** (-1)
        ...
        * (
            ...
            - (tau_m / tau_syn_in) ** (-tau_syn_in / (tau_m - tau_syn_in))
        )
    ) ** (-1)
    ...
    for target_area, target_pop, source_area, source_pop in product(
        ...
        if "E" in source_pop:
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = (
                PSC_e_over_PSP_e * PSP_e
            )
        else:
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = (
                PSC_i_over_PSP_i * g * PSP_e
            )
    ...
    for target_area, target_pop, source_area, source_pop in product(
        ...
        if (
            (lognormal_weights and "E" in target_pop and "E" in source_pop)
            or lognormal_weights
            and not lognormal_EE_only
        ):
    ...
    for area in area_list:
        ...
        synapse_weights_sd[area]["23E"][area]["4E"] = (
            PSC_rel_sd_normal * PSP_e_23_4 * PSC_e_over_PSP_e
        )
    ...
    for target_area, source_area in product(area_list, area_list):
        if source_area != target_area:
            for target_pop, source_pop in product(population_list, population_list):
                ...
                synapse_weights_sd[target_area][target_pop][source_area][
                    source_pop
                ] *= cc_weights_factor
    ...
    with open(
        os.path.join("simulation_result", label, "custom_Data_Model.json"),
        "w",
    ) as f:
