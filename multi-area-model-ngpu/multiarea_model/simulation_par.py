...
from .default_params import check_custom_params, nested_update, sim_params
from .multiarea_helpers import create_vector_mask, extract_area_dict
...
class Simulation:
    def __init__(self, label, network, sim_spec):
        """
        Simulation class.
        ...
        self.data_dir = os.path.join(data_path, "simulation_result", self.label)
        self.copy_files()
        print("Copied files.")
        d = {
        ...
        with open(os.path.join(self.data_dir, "custom_params.json"), "w") as f:
            json.dump(d, f)
    ...
    def __str__(self):
        s = "Simulation {} of network {} with parameters:".format(
            self.label, self.network.label
        )
    ...
    def copy_files(self):
        ...
        """
        files = [
            os.path.join("multiarea_model", "data_multiarea", "Model.py"),
            os.path.join("multiarea_model", "data_multiarea", "VisualCortex_Data.py"),
            os.path.join("multiarea_model", "multiarea_model.py"),
            os.path.join("multiarea_model", "simulation.py"),
            os.path.join("multiarea_model", "default_params.py"),
        ]
        if (
            self.network.params["connection_params"]["replace_cc_input_source"]
            is not None
        ):
            ...
            if ".json" in fs:
                ...
                for it in fn_iter:
                    ...
                    files.append(fp_)
        for f in files:
            shutil.copy2(os.path.join(base_path, f), self.data_dir)
    ...
    def prepare(self):
        ...
        ngpu.SetKernelStatus(
            {
                "rnd_seed": master_seed + ngpu.Rank(),
        ...
        self.pyrngs = [
            ...
            for s in list(range(master_seed + vp + 1, master_seed + 2 * (vp + 1)))
        ]
    ...
    def cortico_cortical_input(self):
        ...
        if self.network.params["connection_params"]["replace_cc_input_source"] is None:
            ...
            replace_cc_input_source = os.path.join(
                self.data_dir,
                self.network.params["connection_params"]["replace_cc_input_source"],
            )
        ...
        if not replace_cc and set(self.areas_simulated) != set(self.network.area_list):
            if replace_non_simulated_areas == "het_current_nonstat":
                fn_iter = model_iter(mode="single", areas=self.network.area_list)
                non_simulated_cc_input = _load_npy_to_dict(
                    replace_cc_input_source, fn_iter
                )
                ...
                non_simulated_cc_input = {
                    source_area_name: {
                        source_pop: self.network.params["input_params"]["rate_ext"]
                ...
                raise KeyError(
                    "Please define a valid method to" " replace non-simulated areas."
                )
        ...
        if replace_cc == "het_current_nonstat":
            ...
            with open(
                self.network.params["connection_params"]["replace_cc_input_source"], "r"
            ) as f:
                ...
                for source_area_name in self.network.area_list:
                    if target_area.name != source_area_name:
                        ...
                        if source_area_name in self.areas:
                            ...
                            connect(self, target_area, source_area)
                        ...
                        # chosen method
                            ...
                            target_area.create_additional_input(
                                replace_non_simulated_areas,
                                source_area_name,
            ...
            for target_area in self.areas:
                for source_area in self.areas:
                    if source_area != target_area:
                        target_area.create_additional_input(
                            replace_cc, source_area.name, cc_input[source_area.name]
                        )
    ...
    def simulate(self):
        ...
        print(
            "Prepared simulation in {0:.2f} seconds.".format(self.time_prepare),
            flush=True,
        )
        ...
        print(
            "Created areas and internal connections in {0:.2f} seconds.".format(
        ...
        if self.areas_recorded == []:
            ...
            for i_interval in range(n_presim_interval):
                ...
                print(
                    "Extracting recorded spike times for presimulation time interval n.",
                    i_interval,
                    "/",
                    n_presim_interval,
                )
            ...
            for i_interval in range(n_interval):
                ...
                print(
                    "Extracting recorded spike times for time interval n.",
                    i_interval,
                    "/",
                    n_interval,
                )
        ...
        for a in self.areas:
            if a.rank == ngpu.Rank():
                for pop in a.populations:
                    ...
                    ngpu.SetStatus(
                        ngpu.NodeSeq(i0, i1 - i0 + 1), "n_rec_spike_times", 0
                    )
        return spike_times_dict
    ...
    def logging(self):
        ...
        fn = os.path.join(
            self.data_dir,
            "recordings",
            "_".join((self.label, "logfile", str(ngpu.Rank()))),
        )
        with open(fn, "w") as f:
            json.dump(d, f)
    ...
    def save_network_gids(self):
        with open(
            os.path.join(self.data_dir, "recordings", "network_gids.txt"), "w"
        ) as f:
...
class Area:
    def __init__(self, simulation, network, name, rank):
        ...
        self.synapses = extract_area_dict(
            network.synapses, network.structure, self.name, self.name
        )
        self.W = extract_area_dict(network.W, network.structure, self.name, self.name)
        self.W_sd = extract_area_dict(
            network.W_sd, network.structure, self.name, self.name
        )
        self.populations = network.structure[name]

        self.external_synapses = {}
        for pop in self.populations:
            self.external_synapses[pop] = self.network.K[self.name][pop]["external"][
                "external"
            ]
    ...
    def connect_populations(self):
        ...
        """
        connect(self.simulation, self, self)
...
def connect(simulation, target_area, source_area):
    ...
    synapses = extract_area_dict(
        network.synapses, network.structure, target_area.name, source_area.name
    )
    W = extract_area_dict(
        network.W, network.structure, target_area.name, source_area.name
    )
    W_sd = extract_area_dict(
        network.W_sd, network.structure, target_area.name, source_area.name
    )
    ...
    for target in target_area.populations:
        for source in source_area.populations:
            ...
            if target_area == source_area:
                if "E" in source:
                    syn_weight.update({"low": 0.0})
                    mean_delay = network.params["delay_params"]["delay_e"]
                elif "I" in source:
                    syn_weight.update({"high": 0.0})
            ...
            syn_spec = {"weight": syn_weight, "delay": syn_delay}
