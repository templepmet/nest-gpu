...
from .default_params import (check_custom_params, complete_area_list,
                             nested_update, network_params)
from .multiarea_helpers import (area_level_dict, convert_syn_weight,
                                dict_to_matrix, dict_to_vector,
                                indegree_to_synapse_numbers, load_degree_data,
                                matrix_to_dict, vector_to_dict)
from .simulation_par import Simulation
...
class MultiAreaModelPar:
        ...
        **keywords
        ...
        if isinstance(network_spec, dict):
            ...
            tmp_parameter_fn = os.path.join(
                "simulation_result", label, "custom_config.json"
            )
            tmp_data_fn = os.path.join(
                "simulation_result", label, "custom_Data_Model.json"
            )

            with open(tmp_parameter_fn, "w") as f:
                json.dump(self.custom_params, f)
            # Execute Data script
            compute_Model_params(label=label, mode="custom")
        else:
        ...
        if self.params["connection_params"]["K_stable"] is None:
            ...
            if not isinstance(self.params["connection_params"]["K_stable"], str):
                raise TypeError(
                    "Not supported. Please store the "
            ...
            ext = {
                area: {pop: ind[area][pop]["external"] for pop in self.structure["V1"]}
        ...
        if self.params["K_scaling"] != 1.0 or self.params["N_scaling"] != 1.0:
            if self.params["fullscale_rates"] is None:
                raise KeyError(
                    "For downscaling, you have to define a file"
        ...
        if isinstance(network_spec, str):
        ...
        if theory:
            ...
            self.init_theory(self.label, theory_spec)
    ...
    def init_theory(self, label, theory_spec):
        self.theory = Theory(label=label, network=self, theory_spec=theory_spec)

    def init_simulation(self, label, sim_spec):
    ...
    def init_analysis(self, ana_spec):
        ...
        self.analysis = Analysis(
            self, self.simulation, data_list=data_list, load_areas=load_areas
        )
    ...
    def scale_network(self):
        ...
        self.K = matrix_to_dict(
            ...
            self.area_list,
            self.structure,
            external=self.K_matrix[:, -1],
        )
        self.W = matrix_to_dict(
            self.W_matrix[:, :-1],
            self.area_list,
            self.structure,
            external=self.W_matrix[:, -1],
        )
    ...
    def vectorize(self):
        ...
        self.J_matrix = convert_syn_weight(
            self.W_matrix, self.params["neuron_params"]["single_neuron_dict"]
        )
        self.structure_vec = [
            "-".join((area, pop))
            for area in self.area_list
            for pop in self.structure[area]
        ]
    ...
    def adj_W_to_K(self):
        ...
        x1 = (
            1e-3
            * tau_m
            * np.dot(self.J_matrix[:, :-1] * self.K_matrix[:, :-1], full_mean_rates)
        )
        K_scaling = self.params["K_scaling"]
        self.J_matrix /= np.sqrt(K_scaling)
        self.add_DC_drive = C_m / tau_m * ((1.0 - np.sqrt(K_scaling)) * (x1 + x1_ext))
        neuron_params = self.params["neuron_params"]["single_neuron_dict"]
        self.W_matrix = 1.0 / convert_syn_weight(1.0, neuron_params) * self.J_matrix
